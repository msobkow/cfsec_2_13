...
MAJOR_VERSION=2
MINOR_VERSION=13
rm -f cfsecramloader*.gz *.deb *.build *.dsc *.log *.changes ex.cmd
pushd cfsecramloader
	if [ -f Makefile ]; then
		make distclean
	fi
popd
pushd cfsecramloader
	...
	sudo chmod 755 /usr/bin/msobkow
	sudo chmod 755 /usr/bin/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}
	sudo chmod 755 /usr/bin/msobkow/${MAJOR_VERSION}.0.${MINOR_VERSION}/cfsecramloader
	sudo ldconfig
popd
