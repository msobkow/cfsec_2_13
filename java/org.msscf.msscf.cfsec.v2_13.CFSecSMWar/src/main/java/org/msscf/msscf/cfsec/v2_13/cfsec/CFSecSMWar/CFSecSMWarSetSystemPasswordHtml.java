...
/**
 * Servlet implementation class CFSecSMWarSetSystemPasswordHtml
 */
@WebServlet("/CFSecSMWarSetSystemPasswordHtml")
public class CFSecSMWarSetSystemPasswordHtml extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public CFSecSMWarSetSystemPasswordHtml() {
		super();
	}
		...
		try {
			...
			if( auth != null ) {
				...
				if( ( passwordHash != null ) && ( passwordHash.length() > 0 ) && ( ! passwordHash.equals( "bootstrap" ) ) ) {
					...
					out.println( "<HTML>" );
					out.println( "<BODY>" );
					out.println( "<form method=\"post\" formaction=\"CFSecSMWarSetSystemPasswordHtml\">" );
			...
			if( auth != null ) {
				...
				if( ( passwordHash != null ) && ( passwordHash.length() > 0 ) && ( ! passwordHash.equals( "bootstrap" ) )  ) {
					...
					if( password.length() <= 0 ) { 
						...
						out.println( "You may now <A HRef=\"CFSecSMWarLoginHtml\">log in to the sec manager.</A>" );
					}
				}
			}
		}
