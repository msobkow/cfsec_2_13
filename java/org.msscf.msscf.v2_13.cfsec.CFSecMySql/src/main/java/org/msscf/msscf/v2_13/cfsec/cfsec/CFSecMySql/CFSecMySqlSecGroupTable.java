// Description: Java 11 MySQL Jdbc DbIO implementation for SecGroup.
...
/*
 *	CFSecMySqlSecGroupTable MySQL Jdbc DbIO implementation
 *	for SecGroup.
 */
public class CFSecMySqlSecGroupTable
	implements ICFSecSecGroupTable
{
	...
	public CFSecMySqlSecGroupTable( CFSecMySqlSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFSecSecGroupBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		...
		int SecGroupId )
	{
		final String S_ProcName = "CFSecMySqlSecGroupTable.readDerivedByIdIdx() ";
		CFSecSecGroupBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecSecGroupBuff readDerivedByUNameIdx( CFSecAuthorization Authorization,
		...
		String Name )
	{
		final String S_ProcName = "CFSecMySqlSecGroupTable.readDerivedByUNameIdx() ";
		CFSecSecGroupBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		boolean IsVisible )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "call " + schema.getLowerDbSchemaName() + ".sp_read_secgrp_by_clustervisidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
		...
		boolean argIsVisible )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "call " + schema.getLowerDbSchemaName() + ".sp_delete_secgrp_by_clustervisidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " )";
			...
			stmtDeleteByClusterVisIdx.executeUpdate();
		}
		catch( SQLException e ) {
			throw new CFLibDbException( getClass(),
				S_ProcName,
