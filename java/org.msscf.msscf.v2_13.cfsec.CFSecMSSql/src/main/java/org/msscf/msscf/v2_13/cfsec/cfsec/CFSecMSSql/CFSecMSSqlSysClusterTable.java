// Description: Java 11 MS SQL Server Developer Edition Jdbc DbIO implementation for SysCluster.
...
/*
 *	CFSecMSSqlSysClusterTable PostgreSQL Jdbc DbIO implementation
 *	for SysCluster.
 */
public class CFSecMSSqlSysClusterTable
	implements ICFSecSysClusterTable
{
	...
	public CFSecMSSqlSysClusterTable( CFSecMSSqlSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFSecSysClusterBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		int SingletonId )
	{
		final String S_ProcName = "CFSecMSSqlSysClusterTable.readDerivedByIdIdx() ";
		CFSecSysClusterBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		int argSingletonId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "exec sp_delete_sysclus_by_ididx ?, ?, ?, ?, ?" + ", "
				+		"?";
