// Description: Java 11 MS SQL Server Developer Edition Jdbc DbIO implementation for SecGroup.
...
package org.msscf.msscf.v2_13.cfsec.cfsec.CFSecMSSql;
...
/*
 *	CFSecMSSqlSecGroupTable PostgreSQL Jdbc DbIO implementation
 *	for SecGroup.
 */
public class CFSecMSSqlSecGroupTable
	implements ICFSecSecGroupTable
{
	...
	public CFSecMSSqlSecGroupTable( CFSecMSSqlSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFSecSecGroupBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		...
		int SecGroupId )
	{
		final String S_ProcName = "CFSecMSSqlSecGroupTable.readDerivedByIdIdx() ";
		CFSecSecGroupBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecSecGroupBuff readDerivedByUNameIdx( CFSecAuthorization Authorization,
		...
		String Name )
	{
		final String S_ProcName = "CFSecMSSqlSecGroupTable.readDerivedByUNameIdx() ";
		CFSecSecGroupBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		boolean IsVisible )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "{ call sp_read_secgrp_by_clustervisidx( ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?" + " ) }";
		...
		int argSecGroupId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "exec sp_delete_secgrp_by_ididx ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?";
		...
		boolean argIsVisible )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "exec sp_delete_secgrp_by_clustervisidx ?, ?, ?, ?, ?" + ", "
				+		"?" + ", "
				+		"?";
			...
			Object stuff = null;
			boolean moreResults = stmtDeleteByClusterVisIdx.execute();
			while( stuff == null ) {
				try {
					moreResults = stmtDeleteByClusterVisIdx.getMoreResults();
				}
				catch( SQLException e ) {
					throw new CFLibDbException( getClass(),
						S_ProcName,
				...
				if( moreResults ) {
					try {
						stuff = stmtDeleteByClusterVisIdx.getResultSet();
					}
					catch( SQLException e ) {
					}
				}
				else if( -1 == stmtDeleteByClusterVisIdx.getUpdateCount() ) {
					break;
				}
			}
		}
