// Description: Java 11 MS SQL Server Developer Edition Jdbc DbIO implementation for ISOCtry.
...
/*
 *	CFSecMSSqlISOCtryTable PostgreSQL Jdbc DbIO implementation
 *	for ISOCtry.
 */
public class CFSecMSSqlISOCtryTable
	implements ICFSecISOCtryTable
{
	...
	public CFSecMSSqlISOCtryTable( CFSecMSSqlSchema argSchema ) {
		schema = argSchema;
	}
	...
	public CFSecISOCtryBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		short ISOCtryId )
	{
		final String S_ProcName = "CFSecMSSqlISOCtryTable.readDerivedByIdIdx() ";
		CFSecISOCtryBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecISOCtryBuff readDerivedByISOCodeIdx( CFSecAuthorization Authorization,
		String ISOCode )
	{
		final String S_ProcName = "CFSecMSSqlISOCtryTable.readDerivedByISOCodeIdx() ";
		CFSecISOCtryBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecISOCtryBuff readDerivedByNameIdx( CFSecAuthorization Authorization,
		String Name )
	{
		final String S_ProcName = "CFSecMSSqlISOCtryTable.readDerivedByNameIdx() ";
		CFSecISOCtryBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		short argISOCtryId )
	{
		...
		try {
			Connection cnx = schema.getCnx();
			String sql = "exec sp_delete_iso_cntry_by_ididx ?, ?, ?, ?, ?" + ", "
				+		"?";
