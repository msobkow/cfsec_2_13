...
package org.msscf.msscf.cfsec.v2_13.cfsec.CFSecOracle;
...
/*
 *	CFSecOracleSecSessionTable Oracle Jdbc DbIO implementation
 *	for SecSession.
 */
public class CFSecOracleSecSessionTable
	implements ICFSecSecSessionTable
{
	...
	public CFSecOracleSecSessionTable( CFSecOracleSchema argSchema ) {
		schema = argSchema;
	}
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {
			...
			stmtCreateByPKey.setString( argIdx++, CFSecOracleSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtCreateByPKey.setString( argIdx++, CFSecOracleSchema.getTimestampString( Finish ) );
			}
			else {
				stmtCreateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
	...
	throws SQLException
	{
		...
		int idxcol = 1;
		CFSecSecSessionBuff buff = schema.getFactorySecSession().newBuff();		buff.setRequiredSecSessionId( CFSecOracleSchema.convertUuidString( resultSet.getString( idxcol ) ) );
		idxcol++;
		...
		idxcol++;
		buff.setRequiredStart( CFSecOracleSchema.convertTimestampString( resultSet.getString( idxcol ) ) );
		idxcol++;
		{
			Calendar colVal = CFSecOracleSchema.convertTimestampString( resultSet.getString( idxcol ) );
			if( resultSet.wasNull() ) {
		...
		idxcol++;
		{
			UUID colVal = CFSecOracleSchema.convertUuidString( resultSet.getString( idxcol ) );
			if( resultSet.wasNull() ) {
	...
	public CFSecSecSessionBuff readDerivedByIdIdx( CFSecAuthorization Authorization,
		UUID SecSessionId )
	{
		final String S_ProcName = "CFSecOracleSecSessionTable.readDerivedByIdIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
	...
	public CFSecSecSessionBuff readDerivedByStartIdx( CFSecAuthorization Authorization,
		...
		Calendar Start )
	{
		final String S_ProcName = "CFSecOracleSecSessionTable.readDerivedByStartIdx() ";
		CFSecSecSessionBuff buff;
		if( ! schema.isTransactionOpen() ) {
			throw new CFLibUsageException( getClass(),
				S_ProcName,
		...
		Calendar Start )
	{
		...
		try {
			...
			stmtReadBuffByStartIdx.setString( argIdx++, CFSecOracleSchema.getTimestampString( Start ) );
		...
		Calendar Finish )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtReadBuffByFinishIdx.setString( argIdx++, CFSecOracleSchema.getTimestampString( Finish ) );
			}
			else {
		...
		UUID priorSecSessionId )
	{
		...
		try {
			...
			if( Finish != null ) {
				stmtPageBuffByFinishIdx.setString( argIdx++, CFSecOracleSchema.getTimestampString( Finish ) );
			}
			else {
		...
		CFSecSecSessionBuff Buff )
	{
		...
		try {			UUID SecSessionId = Buff.getRequiredSecSessionId();
			...
			stmtUpdateByPKey.setString( argIdx++, CFSecOracleSchema.getTimestampString( Start ) );
			if( Finish != null ) {
				stmtUpdateByPKey.setString( argIdx++, CFSecOracleSchema.getTimestampString( Finish ) );
			}
			else {
				stmtUpdateByPKey.setNull( argIdx++, java.sql.Types.VARCHAR );
			}
		...
		Calendar argStart )
	{
		...
		try {
				...
				stmtDeleteByStartIdx.setString( argIdx++, CFSecOracleSchema.getTimestampString( argStart ) );
		...
		Calendar argFinish )
	{
		...
		try {
				...
				if( argFinish != null ) {
					stmtDeleteByFinishIdx.setString( argIdx++, CFSecOracleSchema.getTimestampString( argFinish ) );
				}
				else {
